<script lang="ts">
    ...
    const id = $derived(page.params.uuid);
    const chat = $derived(chats[id]);
    ...
    onMount(() => {
        ...
        if (last?.role === "user") {
            streamReply(id, globalState.model);
        }
    });
</script>
...
<div class="flex h-screen flex-col">
                        ...
                        <div>
                            ...
                            {#if message.done !== false}
                                <MessageActions
                                    ...
                                    onRetry={() => retryLast(id, globalState.model)}
                                />
                            {/if}
                        </div>
                    {/if}
    ...
    <div class="bg-bg-100 pb-8 pt-2">
        <div class="mx-auto flex w-full max-w-2xl justify-center px-6">
            <PromptBox chatId={id} placeholder="Reply..." openUp />
        </div>
    </div>
</div>
